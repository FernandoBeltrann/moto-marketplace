...
export type FaqItem = {
  ...
  answer: string;
  /** Nota de contenido pendiente de confirmar (ej. dato que falta) — se muestra como tag ámbar arriba de la respuesta. */
  pending?: string;
};
          ...
          <div key={item.id} className="faq-accordion__item">
            ...
            <div className={'faq-accordion__answer' + (isOpen ? ' faq-accordion__answer--open' : '')}>
              <div className="faq-accordion__answer-inner">
                {item.pending ? <span className="pending-tag">🔲 {item.pending}</span> : null}
